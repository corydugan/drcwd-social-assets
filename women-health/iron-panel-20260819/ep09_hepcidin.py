...
def frame(f):
    ...
    if out > 0.01:
        ...
        bio.liver(d, 430, 716, 158, alpha=out, draw_p=B.seg(f, 3.0, 5.8),
                  label="LIVER", label_a=B.seg(f, 5.4, 6.4) * out)
        bio.gut(d, 110, 970, GUT_Y, amp=44, alpha=out, draw_p=B.seg(f, 5.6, 8.4))
        a = B.seg(f, 8.0, 9.0) * out
        if a > 0:
            d.text((110, GUT_Y + 92), "GUT", font=B.SANS_B(30), fill=B.mix(B.EYEBROW, a))

        # iron crossing the wall while the door is open
        shut = B.seg(f, 16.6, 19.0)
        for j in range(4):
            t = 9.4 + j * 0.7
        ...
        a = B.seg(f, 19.2, 20.2) * out          # stays: the closing frame is a shut door
        if a > 0:
        ...
        S.narrate(d, f, [
            (3.0,  8.6,  "Your liver makes a hormone that decides how much iron gets in."),
            (8.8,  13.2, "While you need iron, the door in the gut wall stays open."),
            (13.4, 18.6, "Raise the hormone and the door shuts."),
            (18.8, 22.4, "Inflammation raises it, whether or not you need what is behind it."),
        ], out)
